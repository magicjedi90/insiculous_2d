...
### Key Metrics
- **Total Tests**: 1252/1252 passing (100% success rate), 0 ignored
...
- Games (in `../games/`): breakout 47 tests, pong 11, space_invaders 36, snake 38, asteroids 42, frogger 40 — all clippy-clean, all 2-player; Pong is fully localized (en + pirate, locale-driven font); Frogger is the first Tilemap consumer (Jul 2026)

### Current Priority
**The 20 Games Challenge** drives the roadmap (see `PROJECT_ROADMAP.md`): **Phase A complete** — Pong ☑, Breakout ☑, Space Invaders ☑, Snake ☑, Asteroids ☑. **Phase B complete (Jul 2026)** — Gap 1 `CameraFollow` (+ main-camera-entity → render-camera sync), Gap 2 `Lifetime`, Gap 3 `Tilemap` (batched through the sprite pipeline). **Phase C started (Jul 2026)** — game 6 Frogger ☑ shipped (`../games/frogger/`, first Tilemap consumer, 40 tests). **Next: game 7 (Tetris)**. Editor: Phase 1 complete; Phase 2 (Ideal Editor UI) in progress (2F Status Bar done, 2G Theme started).
...
cargo test --workspace               # Run all 1252 tests
